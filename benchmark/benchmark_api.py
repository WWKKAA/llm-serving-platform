...
def build_payload(max_tokens: int) -> Dict[str, Any]:
    return {
        "messages": [
            {
                "role": "user",
...
async def run_benchmark(total_requests: int, concurrency: int, max_tokens: int):
    payload = build_payload(max_tokens=max_tokens)
    ...
    async with httpx.AsyncClient(limits=limits) as client:
        for batch_start in range(0, total_requests, concurrency):
            ...
            tasks = [
                send_one_request(client, request_id=i, payload=payload)
